<template>
    <div>
        ...
        <Menu
            ...
            <template v-else>
                <Tooltip :content="tTitle(item.title)" placement="right" v-if="item.children === undefined || !item.children.length" :key="wwww">
                    <i-menu-side-item :menu="item" hide-title />
                </Tooltip>
                <i-menu-side-collapse v-else :menu="item" :key="wwww" top-level />
            </template>
        </Menu>
    </div>
</template>
